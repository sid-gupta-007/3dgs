...
def test_reconstruct_layout_with_inpainting_and_backing(tmp_path):
    ...
    class MockEstimator(DepthEstimator):
        def estimate(self, image):
            ...
            H, W = img_np.shape[:2]
            d = np.full((H, W), 4.0, dtype=np.float32)
            # Create a foreground object in lower half (chair/table)
            d[int(H * 0.55):int(H * 0.85), int(W * 0.4):int(W * 0.6)] = 1.8
    ...
    # Point cloud should have primary points + infilled floor points + volumetric back surfels
    assert pc.num_points >= 64 * 128
